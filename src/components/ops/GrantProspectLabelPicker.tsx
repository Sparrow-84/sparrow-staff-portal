...
export function GrantProspectLabelPicker({ kind, kindDisplayName, value, currentUserId, onChange }: Props) {
  ...
  return (
    <div ref={wrapRef} className="relative">
      <span className="mb-1 block text-xs font-medium text-sparrow-gray">{kindDisplayName}</span>
      <button
        type="button"
        onClick={openDropdown}
        className="field-input mt-0 flex items-center justify-between text-left"
      >
        {selected ? <LabelPill label={selected.name} color={selected.color} /> : <span className="text-sparrow-gray">Pick or create…</span>}
        ...
        <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-80 overflow-y-auto rounded-lg border border-sparrow-rule bg-white shadow-xl">
                ...
                {labels.map((label) => (
                  <li key={label.id}>
                    ...
                    >
                      ...
                      {value === label.id && <span className="text-sparrow-green">✓</span>}
                    </button>
                  </li>
                ))}
                {labels.length === 0 && (
                  <li className="px-3 py-2 text-xs text-sparrow-gray">No {kindDisplayName.toLowerCase()} labels yet — create one below.</li>
                )}
              </ul>
              <div className="border-t border-sparrow-rule">
                {value && (
                  <button type="button" onClick={clearSelection} className="flex w-full items-center gap-2 px-3 py-2 text-xs text-sparrow-gray hover:bg-sparrow-mist">
                    <span>✕</span> Clear
                  </button>
                )}
                <button type="button" onClick={() => setView('create')} className="flex w-full items-center gap-2 px-3 py-2 text-xs font-medium text-sparrow-green hover:bg-sparrow-mist">
                  <span>+</span> Create {kindDisplayName.toLowerCase()} label
                </button>
                {labels.length > 0 && (
                  <button type="button" onClick={() => { setView('manage'); setEditingId(null); }} className="flex w-full items-center gap-2 px-3 py-2 text-xs text-sparrow-gray hover:bg-sparrow-mist">
          ...
          {view === 'create' && (
            <div className="space-y-3 p-3">
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setView('list')} className="text-xs text-sparrow-gray hover:text-sparrow-ink">← Back</button>
                <span className="text-xs font-semibold text-sparrow-ink">New {kindDisplayName.toLowerCase()} label</span>
              </div>
              <input
                type="text"
                value={createName}
          ...
          {view === 'manage' && (
            <div className="space-y-2 p-3">
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => { setView('list'); setEditingId(null); }} className="text-xs text-sparrow-gray hover:text-sparrow-ink">← Back</button>
                <span className="text-xs font-semibold text-sparrow-ink">Manage {kindDisplayName.toLowerCase()} labels</span>
              </div>
              {labels.map((label) => (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          autoFocus
                        />
                        <button type="button" onClick={() => void handleSaveEdit(label.id)} disabled={!editName.trim() || savingEdit} className="text-xs font-medium text-sparrow-green hover:opacity-70">Save</button>
                        <button type="button" onClick={() => setEditingId(null)} className="text-xs text-sparrow-gray hover:text-sparrow-ink">✕</button>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2">
                      ...
                      <button type="button" onClick={() => startEdit(label)} className="text-sparrow-gray hover:text-sparrow-ink" aria-label="Edit">✎</button>
                      <button type="button" onClick={() => void handleDelete(label.id)} disabled={deletingId === label.id} className="text-sparrow-gray hover:text-priority-p1" aria-label="Delete">🗑</button>
                    </div>
                  )}
                </div>
              ))}
              {labels.length === 0 && <p className="text-xs text-sparrow-gray">No labels to manage yet.</p>}
              <button type="button" onClick={() => setView('create')} className="flex w-full items-center gap-2 pt-1 text-xs font-medium text-sparrow-green hover:opacity-70">
