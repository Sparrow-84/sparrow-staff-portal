...
insert into lcp_units (phase_id, name, sort_order, month_label, artifact, supplement) values
  ...
  ((select id from lcp_phases where number=3), 'Kids'' Bedroom',   6, 'Month 7',   'Blessing Card ceremony',                             'Optional: Parenting with Autism & ADHD (3 sessions)'),
  ((select id from lcp_phases where number=3), 'Master Bedroom',   7, 'Month 6',   'Letter to My Child',                                 null),
  ((select id from lcp_phases where number=3), 'Bathroom',         8, 'Month 5',   'Walking Out drawing',                                null),
