...
insert into lcp_units (phase_id, name, sort_order, month_label, artifact, supplement) values
  ...
  ((select id from lcp_phases where number=3), 'Kids'' Bedroom',   6, 'Month 5',   'Blessing Card ceremony',                             'Optional: Parenting with Autism & ADHD (3 sessions)'),
  ((select id from lcp_phases where number=3), 'Master Bedroom',   7, 'Month 6',   'Letter to My Child',                                 null),
  ((select id from lcp_phases where number=3), 'Bathroom',         8, 'Month 7',   'Walking Out drawing',                                null),
...
insert into lcp_sessions (unit_id, session_number, title, sort_order, focus, scripture) values
  ...
  ((select id from lcp_units where name='Kids'' Bedroom'),   21, 'Who this child is',                         21, 'Child development by stage · secure attachment · the 30% repair rule', null),
  ((select id from lcp_units where name='Kids'' Bedroom'),   22, 'Not the way I was parented',                22, 'Four parenting styles · generational cycle-breaking', null),
  ((select id from lcp_units where name='Kids'' Bedroom'),   23, 'What your child is absorbing',              23, 'Safe? Loved? Capable? · home atmosphere', null),
  ((select id from lcp_units where name='Kids'' Bedroom'),   24, 'Correction that builds',                    24, 'Discipline vs. punishment · four tools', null),
  ((select id from lcp_units where name='Kids'' Bedroom'),   25, 'When children compete',                     25, 'Sibling rivalry · each child uniquely known', null),
  ((select id from lcp_units where name='Kids'' Bedroom'),   26, 'The hard days',                             26, 'Compassion fatigue · the absent other parent · holding authority alone', null),
  ((select id from lcp_units where name='Master Bedroom'),   27, 'He makes me lie down',                      27, 'Rest as spiritual discipline', null),
  ((select id from lcp_units where name='Master Bedroom'),   28, 'The way it was meant to be',                28, 'God''s original design for sexuality', null),
  ((select id from lcp_units where name='Master Bedroom'),   29, 'Covenant, purity and moving forward',       29, 'Purity as protection of worth', null),
  ((select id from lcp_units where name='Master Bedroom'),   30, 'Healing from sexual shame — part 1',        30, 'Naming the wound', null),
  ((select id from lcp_units where name='Master Bedroom'),   31, 'Go in peace',                               31, 'From wound to freedom · the generational reach of healing', null),
  ((select id from lcp_units where name='Bathroom'),         32, 'The mirror',                                32, 'Who do you see? · what do you believe about what you see?', null),
  ((select id from lcp_units where name='Bathroom'),         33, 'The wash',                                  33, 'Confession not as shame but as the mechanism of freedom', null),
  ((select id from lcp_units where name='Bathroom'),         34, 'Walking out clean',                         34, 'New identity · how to hold it when the old one calls', null),
